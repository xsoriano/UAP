		...
		callback: function(result) {
			if (result){
				...
				for (var i = 0;  i < clipBookmarks.length; i++){
					...
					bookmarkssDB.remove(toDeleteBookmarkKey);
				}
...
$(document).ready(function() {
	Session.set('EC_CHECKING_CLIP_SOURCE',false);
	var ec_audio = document.getElementById('ec-audio');
	var ec_source = document.getElementById('ec-source');

	
	ec_audio.addEventListener('loadedmetadata', function() {
		console.log('loadedmetadata');
		Session.set('EC_CHECKING_CLIP_SOURCE',false);
	});

	ec_source.addEventListener('error', function() {
		console.log('error');
