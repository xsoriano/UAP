...
function updateEditingBookmark(name, start, end, notes){
	...
	if (Session.equals('CURRENT_BOOKMARK_KEY', editingBookmark)){
		MixTape.adjustBookmarkMarkers(startMils, endMils);
	}
	bootbox.alert({
		title: "Confirmation",
	...
	bootbox.confirm({
		...
		callback: function(result) {
			if (result){
				...
				if (Session.equals('CURRENT_BOOKMARK_KEY', toDeleteBookmarkKey)){
